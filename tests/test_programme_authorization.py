...
class ProgrammeAuthorizationGateTests(unittest.TestCase):
    ...
    def test_repair_is_limited_per_exact_pr_head_and_new_head_requires_new_qualification(self):
        self.gate.record_authorization(self.authorization)
        ...
        self.assertEqual(self.gate.record_repair_attempt(self.authorization.authorization_id, self.candidate(head_sha="b" * 40, technical_qualification_passed=False, ci_passed=False)), self.gate._decision_digest(f"{self.authorization.authorization_id}:repair:77:{'b' * 40}:1"))
