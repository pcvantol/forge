...
class InstalledForgeUpdateTests(unittest.TestCase):
    ...
    def _normal_release_request(self, version="2.7.24", existing_version="2.7.22"):
        ...
        if version in {"2.7.25", "2.7.26", "2.7.27", "2.7.28", "2.7.29", "2.7.30", "2.7.31"}:
    ...
    def test_2727_to_2728_same_schema_release_preserves_history_and_requires_composition(self):
        before = self._same_schema39_transition(existing_version="2.7.27", target_version="2.7.28")
        self.assertEqual(update.transition_schemas(self.request), (39, 39))
        self.assertEqual(update.validate_qualified_artifact(self.request)["release_route"], "NORMAL")
        controller = self._controller()
        self.assertEqual(controller.backup_path.name, "forge-schema39.sqlite3")
        ...
        receipt = Path(self.request.qualification_receipt)
        document = json.loads(receipt.read_text())
        del document["publication_receipt"]["criterion_completion"]
        receipt.write_text(json.dumps(document, sort_keys=True))
        changed = update.UpdateRequest(**{
            **self.request.__dict__, "qualification_receipt_sha256": update.file_digest(receipt),
        })
    ...
    def test_2729_to_2730_same_schema_release_preserves_history_and_requires_composition(self):
        before = self._same_schema39_transition(existing_version="2.7.29", target_version="2.7.30")
        self.assertEqual(update.transition_schemas(self.request), (39, 39))
        self.assertEqual(update.validate_qualified_artifact(self.request)["release_route"], "NORMAL")
        controller = self._controller()
        self.assertEqual(controller.backup_path.name, "forge-schema39.sqlite3")
    ...
    def test_2730_to_2731_same_schema_release_preserves_history_and_requires_composition(self):
        before = self._same_schema39_transition(existing_version="2.7.30", target_version="2.7.31")
        self.assertEqual(update.transition_schemas(self.request), (39, 39))
        self.assertEqual(update.validate_qualified_artifact(self.request)["release_route"], "NORMAL")
        controller = self._controller()
        self.assertEqual(controller.backup_path.name, "forge-schema39.sqlite3")
    ...
    def test_exact_published_wheel_end_to_end_when_requested(self) -> None:
        ...
        target_schema = 39 if target_version in {"2.7.25", "2.7.26", "2.7.27", "2.7.28", "2.7.29", "2.7.30", "2.7.31"} else 38
        previous_version = {
            "2.7.26": "2.7.25", "2.7.27": "2.7.26", "2.7.28": "2.7.27",
            "2.7.29": "2.7.28", "2.7.30": "2.7.29", "2.7.31": "2.7.30",
        }.get(target_version)
