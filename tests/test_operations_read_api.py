...
class TestTransportContract(unittest.TestCase):
    def test_openapi_and_postman_cover_the_implemented_read_routes(self) -> None:
        ...
        self.assertEqual(set(openapi["paths"]), {"/v1/health", "/v1/status", "/v1/missions/{mission_id}"})
        ...
        for path in openapi["paths"].values():
            for response in path["get"]["responses"].values():
                ...
                references = media["schema"].get("oneOf", (media["schema"],))
                for reference in references:
                    self.assertRegex(reference["$ref"], r"^#/components/schemas/")
        self.assertEqual(
