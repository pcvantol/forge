...
class GovernanceAuthorityTests(unittest.TestCase):
    ...
    def test_canonical_intake_fails_closed_for_malformed_or_conflicting_existing_state(self):
        planning = self.planning()
        CanonicalBusinessWorkspace(self.repository, self.context).approve(
            decision_id="business-state", candidate_id="candidate-state", revision="1", scope=planning.scope, gates=("business",),
        )
        CanonicalArchitectureWorkspace(self.repository, self.context).approve(
            decision_id="architecture-state", candidate_id="candidate-state", revision="1", planning=planning,
        )
        envelope = MissionPlanningEvidenceEnvelope.compose(
            self.repository, subject_id="candidate-state", subject_revision="1",
            business_decision_id="business-state", architecture_decision_id="architecture-state", planning=planning,
        )
        source = "canonical-governance-envelope:" + envelope.digest
        mission_id = self.db.allocate_next_mission_id(source=source, allocated_at="now")
        mission = ArchitectureMission(
            id=mission_id, candidate_id="candidate-state", title="Canonical intake", summary="bounded",
            business_objective="qualify", business_value="evidence", architecture_review_reference="architecture-state",
            mission_recommendation_reference="business-state", scope=("scope",), engineering_constraints=("NONE",),
            acceptance_criteria=("no actions",), technical_assumptions=("runtime owned",), dependencies=("G001",),
            required_capabilities=("planning",), required_disciplines=(RequiredDiscipline.PLATFORM_ARCHITECTURE,),
            risks=("untrusted",), status=ArchitectureMissionStatus.APPROVED_FOR_ENGINEERING,
        )
        intake = MissionIntake(None, lambda: "now")
        intake.admit_canonical_approved_mission(mission, envelope, self.repository)
        document = self.db.get_document("mission_state", mission_id)
        with self.db._connection:
            self.db._connection.execute("UPDATE mission_state SET document = ? WHERE mission_id = ?", ("not-json", mission_id))
        with self.assertRaisesRegex(MissionIntakeError, "could not read canonical Mission state"):
            intake.admit_canonical_approved_mission(mission, envelope, self.repository)
        document["admission_contract"]["candidate_id"] = "conflicting-candidate"
        self.db.save_mission_state(document)
        with self.assertRaisesRegex(MissionIntakeError, "conflicting canonical Mission state"):
            intake.admit_canonical_approved_mission(mission, envelope, self.repository)
        self.assertEqual(self.db.allocate_next_mission_id(source=source, allocated_at="later"), mission_id)
