...
This section adopts the detailed, dependency-aware [implementation sequence](../../docs/roadmap/0.1.md) introduced on PR #7. It does not approve a Mission, allocate a Mission ID, or authorize execution. Each increment requires its own approved Mission and must preserve the Forge/Workspace/EP ownership boundaries. The [V1 gap register](../../docs/architecture/FORGE_WORKSPACE_V1_GAP_REGISTER.md) records unresolved cross-product decisions.

| Canonical increment | V1 review mapping and remaining cross-product evidence |
| --- | --- |
| L0 — Engineering Contract Foundation | Implements the documented Effective DoR/DoD/Human Gate and immutable Action snapshot target (`FWV1-G002/G006`). EP-owned; its versioned consumer contract, Goldens and enforcement qualification must precede dependent work. |
| L1 — Learning Evidence + New-Project Bootstrap Contract | Implements locally packaged baseline, project-owned contract, Action evidence envelope and clean-install behavior (`G002/G010`). Forge/EP boundary. |
| L1-R — Managed Repository Governance Baseline | Implements desired state, GitHub provision/read-back and adoption/drift behavior (`G003/G004`). It cannot begin managed bootstrap until the EP accepted-request/attachment authority and Workspace intent contract are frozen. |
| L2–L4 — Quality learning and Workspace governance | Implements the documented quality observer/review/governance surfaces. Foundation-only: it must not block normal execution or silently mutate project policy. |
| L5–L10 — Knowledge learning and release qualification | Implements governed KB export/consumption and proves learning effectiveness/self-contained release. KB remains additive; L10 is the required final clean-install/no-source-authority qualification. |

Cross-product work still not fully represented by the L-sequence—installation/named operator (`G001`), multi-repository lease/coordination (`G005`), history/attention recovery (`G007`), lifecycle retention (`G008/G009`), V1 security (`G011`) and responsive Workspace UI (`G013`)—requires separately bounded dependencies before a V1 implementation-ready claim. Only work that does not mutate a shared schema/API is parallel-safe; a shared-contract predecessor must be reviewed and qualified first.
