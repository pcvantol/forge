...
MISSION_RECOMMENDATION_SCHEMA_VERSION = "1.0"
...
class MissionRecommendation:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != MISSION_RECOMMENDATION_SCHEMA_VERSION or not all((self.id, self.architecture_review_id, self.repository_maturity_digest, self.title, self.rationale, self.business_value, self.architectural_value, self.recommendation_timestamp)):
        ...
        for name in ("required_disciplines", "missing_disciplines", "capability_impact", "source_signal_ids", "portfolio_item_ids"):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"schema_version": self.schema_version, "id": self.id, "repository_context": self.repository_context.to_dict(),
                ...
                "source_signal_ids": list(self.source_signal_ids), "portfolio_item_ids": list(self.portfolio_item_ids), "advisory": True}
